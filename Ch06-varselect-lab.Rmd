...
```
Recall we set up the `kfold` object for 5-fold cross-validation on page 6.5.1. We now plot the cross-validated MSE as a function of $-\log(\lambda)$, which has shrinkage decreasing from left
to right.
...
regression (page 6.5.2) with $\lambda$ chosen by cross-validation.
